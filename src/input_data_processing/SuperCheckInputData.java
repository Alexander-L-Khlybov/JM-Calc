...
import primitives.SuperAllowedCharacters;
...
public class SuperCheckInputData {
    public static void checkInputData (String inputData) throws Exception {
        ...
        for (int i = 0; i < inputData.length(); i++){
            if(!SuperAllowedCharacters.isAllowedCharacter(inputData.charAt(i)))
        ...
        int tmp = 0;
        for (int i = 0; i < inputData.length(); i++){
            if(SuperAllowedCharacters.isOperation(inputData.charAt(i)))
                tmp++;
        }
        if (tmp == 0)
        ...
        if (SuperAllowedCharacters.isOperation(inputData.charAt(0)) &&
                (inputData.charAt(0) != '-') && (inputData.charAt(0) != '('))
            throw new Exception("The operation character cannot be at the beginning of the line.");
        if (SuperAllowedCharacters.isOperation((inputData.charAt(inputData.length() - 1))))
        ...
        for (int i = 0; i < inputData.length(); i++){
            if (SuperAllowedCharacters.isArabic(inputData.charAt(i)))
                arabic = true;
            else if (SuperAllowedCharacters.isRoman(inputData.charAt(i)))
                roman = true;
        }
        if (arabic && roman)
